...
class TimelineGraphView(BranchingProcessView):
    ...
    def __init__(self, model: BranchingProcessModel):
        ...
        self._node_type_counts = pd.DataFrame(columns=[node.name for node in self._model.network.node_colours.keys()])
    ...
    def draw_timeline(self):
        ...
        """
        colours = {node.name: colour.colour for node, colour in self._model.network.node_colours.items()}
    ...
    def _increment_timeline(self, network):
        ...
        """
        node_counts = {node.name: network.count_nodes(node) for node in self._model.network.node_colours.keys()}
