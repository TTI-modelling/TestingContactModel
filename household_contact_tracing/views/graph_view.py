...
class GraphView(BranchingProcessView):
    ...
    def draw_network(self):
        ...
        node_colour_map = [network.node_colours[node.node_type()].colour for node in network.all_nodes()]
        edge_colour_map = [network.edge_colours[edge].colour for edge in network.edge_types()]
        ...
        lines = [Line2D([0, 1], [0, 1], color=clr.colour, lw=1) for clr
                 in network.edge_colours.values()]
        labels = [value.label for value in network.edge_colours.values()]
        first_legend = plt.legend(lines, labels, loc="upper left", bbox_to_anchor=(1, 1),
                                  title="Edges")
        plt.gca().add_artist(first_legend)

        # Legend for node colours
        circles = [Circle((0, 0), color=clr.colour, lw=1) for clr in network.node_colours.values()]
        labels = [value.label for value in network.node_colours.values()]
