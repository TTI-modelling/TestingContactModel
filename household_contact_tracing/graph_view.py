...
class GraphView(SimulationViewInterface):
    ...
    def model_simulation_stopped(self, subject: SimulationModelInterface):
        ...
        if self not in subject._observers_graph_change:
            self.draw_network(subject.network, subject.node_type)
    ...
    def draw_network(self, network: Network, node_type: Callable[[Node], str]):
        """Draws the network generated by the model."""

        node_colour_map = [self.node_type_colours[node_type(node)] for node in network.all_nodes()]
